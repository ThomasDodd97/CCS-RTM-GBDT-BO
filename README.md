...
The sizeable dataset generated by Omphalos was used to train a number of [XGBoost](https://xgboost.readthedocs.io/en/stable/) GBDT models to predict target values (carbonates generated within the fracture after a set period of time) from predictors (fluid species composition). Four surrogate models were generated, each subsequent model having been trained for an order of magnitude greater 'time' (n.b. epochs). This dataset and the four associated models can all be downloaded from a Zenodo repository [here](https://zenodo.org/record/6774384#.Yr1ntuzMJb_). To carry out the training, validation, and optimisation yourself you can use code located the in the directory "XGB_GBDT". This directory contains:
* A training notebook "bfm_5-5_1-XGBTraining.ipynb", where data is imported from the .pkl file, cleaned, partitioned into test and evaluate batches, and then passed to the XGBoost module to carry out training.
* A 2D validation notebook "bfm_5-5_2-XGB_2DValidationComparison.ipynb" where all the XGBModels' abilities to predict carbonate values based on variations in a single predictor are compared against RTM ground truth values.
* A 3D validation notebook "bfm_5-5_3-XGB_3DValidationComparison.ipynb" where the XGBModels' abilities to predict based on variation of two parameters is evaluated.
* An optimisation notebook "bfm_5-5_4-XGB-Optimisation" where the bayesian optimisation module is used to optimise specific models.
* An optimisation comparison notebook "bfm_5-5_5-XGB-OptimisationComparison" which cross-compares the various models and their optimisation progression as iterations pass in the bayesian optimisation method.

Each of the surrogate models was treated as black-box functions that needed evaluating and a global optimum finding; a bayesian optimisation framework was used for this purpose. In this case, a pre-built package developed on GitHub entitled ['BayesianOptimization'](https://github.com/fmfn/BayesianOptimization) was implemented in the local python environment to carry out this optimisation procedure.
